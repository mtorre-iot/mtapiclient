...
namespace gaihcc2dataserver
{
    public class App
    {
        ...
        public App(ConcurrentQueue<List<Record>> webhookQueue, CycleTimer cycleTimer, JObject vars, AppSettings config)
        {
            ...
            this.config = config;
            this.vars = vars;
            this.GAIBufferArray = new Dictionary<(string, string), CircularBuffer<double>>();
                        ...
                        while (true)
                        {
                            try
                            {
                                if (webhookQueue.TryDequeue(out List<Record> records) == true)
                                {
                                    ...
                                    {
                                        var valueStr = vqts.vqt.v.ToString();
                                        if (JsonArrayChecker.IsJsonArray(valueStr) == true)
                                        {
                                            double[] values_list = JsonConvert.DeserializeObject<double[]>(valueStr);
                                            //Logger.write(logLevel.info, $"Tag: {vqts.tag}, Max: {values_list.Max()}, Min: {values_list.Min()}");
                                            try
                                            {
                                                var gaibuf = GAIBufferArray[(record.topic, vqts.tag)];
                                                if (gaibuf != null)
                                                {
                                                    gaibuf.Enqueue(values_list, client.interleave); 
                                                }
                                            }
                                            catch (KeyNotFoundException e)
                                            {
                                                Logger.write(logLevel.debug, $"Error trying to get data from buffer. Error: {e.Message}");
                                            }
                                        }
                                        else
                                        {
                                            double value = Convert.ToDouble(valueStr);
                                            Logger.write(logLevel.info, $"Tag: {vqts.tag}, Value: {value}");
                                        }                                        }
